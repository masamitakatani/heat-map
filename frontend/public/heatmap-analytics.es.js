...
function reduceEventData(data) {
  const pendingEvents = {
    clicks: data.pendingEvents.clicks.slice(-50),
    // 最新50件のみ保持
    scrolls: data.pendingEvents.scrolls.slice(-50),
    mouseMoves: data.pendingEvents.mouseMoves.slice(-100)
    // マウス移動は多めに保持
  };
  return {
    ...data,
...
function createInitialData(anonymousId, sessionId) {
  return {
    overlayState: {
      isVisible: false,
      mode: "click"
...
class HeatmapAnalytics {
  ...
  init() {
    ...
    if (this.config.api) {
      try {
        initApiClient(this.config.api);
        initQueryClient();
        registerOnlineListener();
        registerGlobalErrorHandler();
        if (this.config.debug) {
          console.log("[Heatmap] API統合を初期化しました", this.config.api);
        }
      } catch (error) {
        console.error("[Heatmap] API初期化エラー:", error);
      }
    }
    let data = loadData();
    if (!data) {
...
export {
  HeatmapAnalytics as default
};
//# sourceMappingURL=heatmap-analytics.es.js.map
