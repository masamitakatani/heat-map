...
import type { LocalStorageData, PendingEvents } from '../types';
 ...
 */
export function loadData(): LocalStorageData | null {
  try {
 ...
 */
export function saveData(data: LocalStorageData): boolean {
  try {
 ...
 * 古いイベントデータを削除（FIFO方式）
 */
function reduceEventData(data: LocalStorageData): LocalStorageData {
 ...
 * 初期データを作成
 */
export function createInitialData(anonymousId: string, sessionId: string): LocalStorageData {
  return {
    overlayState: {
      isVisible: false,
      mode: 'click',
 ...
 * データの整合性チェック
 */
export function validateData(data: unknown): data is LocalStorageData {
  if (typeof data !== 'object' || data === null) return false;

  const d = data as Partial<LocalStorageData>;
