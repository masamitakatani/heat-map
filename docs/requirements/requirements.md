...
- **導入方法**: Googleタグマネージャー（GTM）経由で配信（推奨）
  - **理由**: コネクティッドワンのページビルダーは、セキュリティ上の理由からカスタムJavaScriptの保存を制限している
  - **代替方法**: 直接`<script>`タグで埋め込み（保存制限がないプラットフォームの場合）
- **管理画面**: なし(LP上で直接可視化)
- **ホスティング**: Vercel CDN経由で配信
...
### 7.4 コネクティッドワン統合の制約
- **問題**: コネクティッドワンのページビルダーは、カスタムJavaScriptの保存を制限（ホワイトリスト方式）
- **保存可能**: Googleタグ、Facebookピクセル等のホワイトリスト登録済みスクリプト
- **保存不可**: `<script>`タグによるカスタムコード（ヘッダー、ボディ、フッター、HTMLブロック）
- **解決策**: Googleタグマネージャー（GTM）を経由してヒートマップライブラリを配信
  - GTMはコネクティッドワンで公式サポート済み
  - エンドユーザー全員のデータ収集が可能
  - 規約違反リスクなし
...
Googleタグマネージャー
  ↓ カスタムHTMLタグで配信
Vercel CDN (heatmap-analytics.umd.js)
  ↓ 初期化
エンドユーザーのデータ収集開始
```
...
#### お客様側の作業
1. GTMアカウント作成（5分、無料）
2. GTMコンテナIDを取得（例: GTM-ABC123）
3. コネワンの「Googleタグ」欄にGTMコードを貼り付け ✅保存可能

#### 納品者側の作業
1. お客様からGTMへのアクセス権を取得
2. GTMでカスタムHTMLタグを作成
3. ヒートマップライブラリの読み込みコードを設定
...
- [GTM統合ガイド](../GTM_INTEGRATION.md) - 技術詳細
- [お客様向けマニュアル](../CUSTOMER_MANUAL.md) - セットアップ手順
- [GTMコードサンプル](../gtm-codes/) - カスタムHTMLタグコード
...
- v3.0 (2025-11-03): GTM統合仕様を追加。コネクティッドワンのJavaScript保存制限問題の解決策を定義
